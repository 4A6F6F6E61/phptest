    ...
    if(isset($_POST['login-submit']))
    {
        ...
        if($count == 1)
        {
            ...
            if(password_verify($_POST['login-password'], $row['PASSWORD']))
            {
                ...
                header('Location: home?alert=Wrong%20password');
            }
        } else {
            header('Location: home?alert=Users%20does%20not%20exist');
        }
    }
    if(isset($_POST['register-submit']))
    {
        ...
        if($count == 0)
        {
            if(isset($_POST['register-username']) && $_POST['register-password'] == $_POST['register-password2'])
            {
                ...
                $st->execute();
                header('Location: home?alert=Account%20successfully%20created');
            } else {
                header('Location: home?alert=Password\'s%20don\'t%20match');
            }
        } else
            header('Location: home?alert=Username%20not%20available');
    }
